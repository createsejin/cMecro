...
namespace sql_executive
{
    ...
    auto prepare_stmt(const char* sql) -> sqlite3_stmt* {
        sqlite3_stmt* stmt;
        const auto rc = sqlite3_prepare_v2(mecro_data, sql, -1, &stmt, nullptr);
        if (rc != SQLITE_OK) {
    ...
    void insert_key_code() {
        ...
        sqlite3_stmt* stmt = prepare_stmt(sql); if (stmt == nullptr) return;
        ...
        stmt = prepare_stmt(sql); if (stmt == nullptr) return;
    ...
    void insert_key_code_oldv() {
        ...
        for (const auto&[key, value] : key_code_map) {
            ...
            if (sqlite3_step(stmt) != SQLITE_DONE) {
                std::cerr << "Failed to execute SQL statement: " << sqlite3_errmsg(mecro_data)
                << " \u25A1" << std::endl;
                sqlite3_finalize(stmt);
                sqlite3_close_v2(mecro_data);
                return;
